...
| 快捷键              | 功能                    | 说明                                                                    |
| :------------------ | :---------------------- | :---------------------------------------------------------------------- |
| `Ctrl+K`            | 打开命令面板            | 全局搜索 / 页面与课程跳转，可搜索并执行命令                             |
| `Ctrl+Enter`        | 运行当前代码            | 在工作区执行编辑器中打开的代码                                          |
| `Ctrl+Shift+Enter`  | 提交代码                | 在工作区提交代码（练习/题目模式）                                       |
| `Alt+1` ... `Alt+8` | 切换主视图              | 1 首页 / 2 课程 / 3 题库 / 4 工作区 / 5 AI / 6 复习 / 7 知识库 / 8 设置 |
| `Esc`               | 关闭 AI 面板 / 命令面板 | 关闭 AI 助手面板、命令面板等对话框                                      |

> macOS 用户：上述 `Ctrl` 对应 `Cmd` 键，`Alt` 即 `Option`，应用会自动识别。
...
3. **命令面板测试**：按 `Ctrl+K`（macOS 为 `Cmd+K`）打开命令面板，使用 `↑`/`↓` 导航，`Enter` 执行，`Escape` 关闭。
