...
Windows x64；macOS 和 Linux 配置可用于源码构建，但在具备同等级打包、安装和运行验证前，
不得作为正式 Release 资产上传。
...
精确的 release tag commit
  ...
  +-- verify-windows-package   -> unsigned Authenticode, install/runtime/uninstall, manifests, hashes
```
...
本地可以生成与正式发布相同签名策略的未签名包，用于构建和安装 smoke：
...
不要把本地产物或普通 CI 产物改名后上传 Release。正式发布必须从精确 release SHA 在 GitHub
Windows runner 上重新构建，固定 `CSC_IDENTITY_AUTO_DISCOVERY=false`，并让验证器确认所有相关
EXE 的 Authenticode 状态严格为 `NotSigned`。
...
| `release-manifest.json`                       | 源 commit、版本、签名状态和产物审计记录 |
...
- GitHub Environment `release` 已建立，并配置适用的审批人和部署限制；
- 发布模式明确为 `unsigned`，`CSC_IDENTITY_AUTO_DISCOVERY=false`，且没有向构建注入签名证书、
  密码、证书名称或预期指纹；
...
缺少这些外部条件时，本地打包仍只能用于开发验证，不能称为正式发布。

## 正式发布门禁

任何一项失败都必须停止发布，不得降级为警告，也不得静默改用另一个 commit、不同签名策略
或较弱的运行验证。
...
### 3. 未签名 Authenticode 策略

- 当前正式 Windows 构建固定为 `unsigned`，并禁止自动发现或注入任何签名证书。
- 安装程序、Portable、`win-unpacked/CodeHelper.exe`、安装后的应用、卸载程序和 Windows Job
  Host 的 Authenticode 状态必须精确为 `NotSigned`。
- `UnknownError`、`HashMismatch`、意外出现的 `Valid` 签名，或任意 signer/timestamp 证书信息都会
  使发布失败；未签名不等于跳过 Authenticode 检查。
- Windows Job Host 仍必须通过 x64 PE 校验，并把大小、SHA-256 和 `NotSigned` 证据记录进清单。
...
- `release-manifest.json` 记录版本、release SHA、workflow commit、生成时间、
  `signatureRequired=false`、各 EXE 的 `NotSigned` 证据、文件大小与 SHA-256；`SHA256SUMS.txt` 覆盖安装程序、blockmap、Portable 和
  `latest.yml`。
...
2. **Build**：重新复核 tag/SHA，执行 Electron harness，在明确的未签名环境中构建并验证。
...
| 变量                              | 正式值     | 约束                                   |
| --------------------------------- | ---------- | -------------------------------------- |
| `CODEHELPER_WINDOWS_RELEASE_MODE` | `unsigned` | 工作流预检必须精确匹配                 |
| `CSC_IDENTITY_AUTO_DISCOVERY`     | `false`    | 禁止 electron-builder 自动选择本机证书 |
| `CODEHELPER_REQUIRE_SIGNATURE`    | `0`        | 清单必须记录 `signatureRequired=false` |

`CSC_LINK`、`CSC_KEY_PASSWORD`、`WIN_CSC_*`、`CSC_NAME` 和 signer thumbprint 在当前模式下必须
保持未设置；意外注入会使发布预检失败。
