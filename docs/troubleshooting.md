...
**原因**: 编辑器（CodeMirror）首次加载需要拉取语言扩展。
...
3. 完全退出应用，按 [备份与恢复手册](guides/backup-restore-runbook.md) 复制整个 `userData`，再检查启动日志中的数据库恢复提示
4. 不要直接删除数据库；损坏文件应先隔离保留，以便恢复尚未导出的题目、草稿和聊天数据

确认 SQLite 损坏时，CodeHelper 会自动运行隔离恢复：原始 DB/WAL/SHM 会保存为 `.corrupt.<timestamp>` 文件，新数据库启动后顶部横幅会显示主备份路径。关闭横幅只确认已知晓，不会删除任何备份。

### 编辑器工作区升级或恢复异常

工作区 localStorage v1/v2/v3 会原地升级到 v4，SQLite editor schema v1/v2 会在事务内升级到 v3。升级后显示“工作区已恢复/标签已恢复”表示未检测到损坏且内容已从 SQLite 或恢复区找回；显示“恢复降级”表示检测到损坏、只能确认 SQLite 中的已有记录可用，或已打开默认/部分工作区，不能推断损坏数据中没有未同步内容；“仅保存在恢复区”表示 SQLite 尚未同步完成，不能当作完整保存。

升级失败时不要删除 `codehelper.db`，也不要清空 localStorage。先完全退出应用并复制整个
`userData`，从而同时保留 DB/WAL/SHM、`.corrupt.*` 文件和 localStorage 中的
`codehelper-editor-workspace.migration-backup.*` 键，再记录恢复/降级 UI 文案和启动日志。旧
practice-backed 标签内嵌代码会转换为普通 `recovered-exercise-*` 文件；如果恢复文件写入失败，
迁移事务会回滚并保留原始行，手工清理反而可能破坏这条回退路径。

关闭窗口时若提示“内容仅保存在恢复区”，优先返回应用并等待 SQLite 恢复；确需关闭时，下次启动应先确认恢复提示和目标标签内容，再继续编辑。练习切题后仍出现“草稿仅本地保存”，表示上一题的最新草稿尚未进入 SQLite，不能把当前新题的正常状态当作旧题已保存。关闭这类练习标签时会再次确认，即使它已是非活动标签：版本冲突提示要求之后重新打开并选择版本，SQLite 写入失败提示则说明恢复区仍可用。多窗口远程关闭当前练习标签时，本窗口会先保存草稿并显示“数据库已保存”或“仅本地恢复区”通知；若数据库和恢复区都不可写，标签会保留在本窗口。

若顶部出现“界面进程异常退出，CodeHelper 已自动重新加载”，主进程仍是原实例，崩溃窗口已由相同位置和尺寸的替代窗口接管。先检查工作区底部状态：只有“工作区已保存”表示 SQLite 已完成同步；“工作区仅本地保存”或“恢复降级”表示仍依赖恢复区，此时不要清理 localStorage 或数据库文件。10 秒内连续崩溃超过 3 次会停止自动循环并显示重新加载/关闭窗口对话框；保留该提示、启动日志和 userData 后再排查显卡驱动、内存压力或损坏资源。
