...
这一版聚焦桌面端可靠性、数据安全和交互一致性：补强工作区恢复、进程清理、AI 请求取消和 IPC 限流，修复语言切换覆盖代码、会话切换污染消息以及 E2E 退出卡死等问题，并将确认框、加载态和主题可读性统一到设计系统。
...
- 工作区与草稿恢复更可靠：语言切换不再覆盖代码，光标/滚动位置、标签拓扑、多窗口同步和异常退出恢复均有持久化回归覆盖。
- 进程生命周期更稳健：代码运行 utility、Windows Job Host 和测试 Electron 进程都采用有界清理，避免并发槽位泄漏或 teardown 无限等待。
- AI 对话更可控：增加 IPC middleware、并发上限、流式 chunk 合并、取消竞态保护和会话切换隔离，网络抖动时保留已有会话列表。
- 数据库迁移更安全：记录实际 schema 指纹，迁移前执行验证型备份；资源包导入、生产测试 hook 和 renderer 路径信任边界进一步收紧。
- UI 体验统一：删除确认改用设计系统对话框，补齐加载态、恢复提示、浅色主题可读性和通用 UI 组件。
- 工程门禁完整：`npm audit` 高危级别为 0，TypeScript、ESLint、Prettier、Vitest、Electron E2E 和生产构建全部通过。
